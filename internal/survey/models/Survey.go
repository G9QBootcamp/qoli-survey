package models

import (
	"time"
	//"github.com/G9QBootcamp/qoli-survey/internal/user" // Correct import path
	"gorm.io/gorm"
)

type Survey struct {
	gorm.Model
	OwnerID            uint      `gorm:"not null"`
	Title              string    `gorm:"not null"`
	StartTime          time.Time `gorm:"not null"`
	EndTime            time.Time `gorm:"not null"`
	IsSequential       bool      `gorm:"default:false"`
	AllowReturn        bool      `gorm:"default:false"`
	ParticipationLimit int       `gorm:"default:1"`
	AnswerTimeLimit    int       `gorm:"not null"`
	//Owner              user.User  `gorm:"foreignKey:OwnerID;references:ID;constraint:OnDelete:CASCADE;"` // Correct model name
	Questions []Question `gorm:"foreignKey:SurveyID;constraint:OnDelete:CASCADE;"`
}
